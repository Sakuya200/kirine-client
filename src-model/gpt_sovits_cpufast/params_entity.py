...
class CommonTaskArgs:
	...
	def from_mapping(cls, value: dict[str, object]) -> "CommonTaskArgs":
		...
		if raw_model_params is None:
			...
			raise TypeError("Malformed params payload: model_params_json must be an object")
...
class VoiceCloneArgs:
	...
	def from_mapping(cls, value: dict[str, object]) -> "VoiceCloneArgs":
		return cls(
			common=CommonTaskArgs.from_mapping(value),
			ref_audio_path=_coerce_required_str(value.get("ref_audio_path"), "args.VoiceClone.ref_audio_path"),
...
class ParamsEntity:
	...
	def from_dict(cls, payload: dict[str, object]) -> "ParamsEntity":
		...
		if kind is TaskKind.TEXT_TO_SPEECH:
			parsed_args: TextToSpeechArgs | VoiceCloneArgs = TextToSpeechArgs.from_mapping(nested_args)
		elif kind is TaskKind.VOICE_CLONE:
			parsed_args = VoiceCloneArgs.from_mapping(nested_args)
		else:
			raise ValueError("GPT-SoVITS-CPUFast package only supports TextToSpeech and VoiceClone")
	...
	def ensure_kind(self, expected: TaskKind) -> "ParamsEntity":
		if self.kind is not expected:
			raise ValueError(f"Expected {expected.value} params payload, got: {self.kind.value}")
		return self
