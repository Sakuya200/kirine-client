...
from gpt_sovits_v2pp.params_entity import ParamsEntity, RuntimeOptions


GPT_SOVITS_V2PP_VERSION = "v2ProPlus"
GPT_SOVITS_V2PP_BASE_MODEL = "gpt_sovits_v2pp"


def _require_path(path: str | None, label: str) -> Path:
    if path is None or not str(path).strip():
        raise ValueError(f"GPT-SoVITS-V2PP params payload is missing {label}")
...
def _resolve_bridge_script_path() -> Path:
    ...
    if not bridge_script_path.exists():
        raise FileNotFoundError(f"GPT-SoVITS-V2PP bridge script not found: {bridge_script_path}")
...
def _resolve_cpufast_root(model_root_path: str | None) -> Path:
    ...
    candidate_roots = [
        root_path,
        root_path / GPT_SOVITS_V2PP_BASE_MODEL,
        root_path / "base-models" / GPT_SOVITS_V2PP_BASE_MODEL,
...
def _map_runtime_language(language: str | None) -> str:
    ...
    if normalized not in mapping:
        raise ValueError(f"Unsupported GPT-SoVITS-V2PP language: {language}")
...
@dataclass
class GptSovitsV2ppVoiceCloneParams:
    bridge_script_path: Path
    cpufast_root: Path
    gpt_model_path: Path
    sovits_model_path: Path
...
def load_tts_params(path: str | Path) -> GptSovitsV2ppTtsParams:
...
def load_voice_clone_params(path: str | Path) -> GptSovitsV2ppVoiceCloneParams:
    params = ParamsEntity.from_file(path)
    args = params.voice_clone_args()
    cpufast_root = _resolve_cpufast_root(args.common.model_root_path)
    runtime = _normalize_runtime(params.runtime)

    return GptSovitsV2ppVoiceCloneParams(
        bridge_script_path=_resolve_bridge_script_path(),
        cpufast_root=cpufast_root,
        gpt_model_path=_resolve_default_gpt_path(cpufast_root),
        sovits_model_path=_resolve_default_sovits_path(cpufast_root),
...
def run_inference_cli(args: Namespace) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="gpt-sovits-v2pp-") as temp_dir:
        ...
        if not final_output_path.exists():
            raise FileNotFoundError(
                f"GPT-SoVITS-V2PP output file not found: {final_output_path}"
            )