...
class TaskKind(str, Enum):
    ...
    STREAMING_SPEECH = "StreamingSpeech"
...
# 顶层 ``kind`` 字段值（PythonScriptTaskKind 序列化）与 ``args`` 嵌套键
# （PythonScriptTaskArgs 变体名）在前 4 类恰好相同，但流式不同：
...
_ARGS_KEY_BY_KIND: dict["TaskKind", str] = {
    ...
    TaskKind.STREAMING_SPEECH: "Streaming",
}
...
class VoiceDesignArgs:
    ...
    def from_mapping(cls, value: dict[str, object]) -> "VoiceDesignArgs":
        return cls(
            ...
            output_path=_coerce_required_str(value.get("output_path"), "args.VoiceDesign.output_path"),
        )
...
class StreamingArgs:
    ...
    speakers 不在此解析——streaming.py 从 ``context.json`` 的 ``basic.speakers`` 取
    说话人列表（含 ref_audio / speaker_dir_name / category 等完整字段）；此处仅取
    路径与 ``model_root_path``（trained 说话人 checkpoint 解析所需）。
    """
    ...
    def from_mapping(cls, value: dict[str, object]) -> "StreamingArgs":
        return cls(
            common=CommonTaskArgs.from_mapping(value),
            context_file_path=_coerce_required_str(
                value.get("context_file_path"), "args.Streaming.context_file_path"
            ),
            input_cache_file_path=_coerce_required_str(
                value.get("input_cache_file_path"), "args.Streaming.input_cache_file_path"
            ),
            output_audio_dir=_coerce_required_str(
...
class ParamsEntity:
    ...
    def from_dict(cls, payload: dict[str, object]) -> "ParamsEntity":
        ...
        elif kind is TaskKind.STREAMING_SPEECH:
            parsed_args = StreamingArgs.from_mapping(nested_args)
        else:
    ...
    def streaming_args(self) -> StreamingArgs:
        self.ensure_kind(TaskKind.STREAMING_SPEECH)
        if not isinstance(self.args, StreamingArgs):
            raise TypeError("Malformed params payload: streaming args were not parsed")
        return self.args
