...
install_compatible_torch_cuda() {
    ...
    for candidate_tag in $candidates; do
        if run_checked "install torch wheels for cu$candidate_tag" "$VENV_PYTHON" -m pip install --force-reinstall --no-cache-dir torch==2.10.0 torchvision torchaudio --index-url "https://download.pytorch.org/whl/cu$candidate_tag" \
