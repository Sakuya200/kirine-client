...
function Install-CudaTorchRuntime {
    ...
    foreach ($candidate in $candidates) {
        ...
        try {
            Invoke-LoggedCommand -Description "install torch CUDA wheels (cu$tag)" -Command $venvPython -Arguments @('-m', 'pip', 'install', '--upgrade', '--force-reinstall', '--no-cache-dir', 'torch', 'torchvision', 'torchaudio', '--index-url', "https://download.pytorch.org/whl/cu$tag")
