...
function Install-CompatibleTorchCuda {
    ...
    foreach ($candidate in $CudaCandidates) {
        ...
        try {
            Invoke-LoggedCommand -Description "install torch wheels for cu$candidateTag" -Command $venvPython -Arguments @('-m', 'pip', 'install', '--force-reinstall', '--no-cache-dir', 'torch==2.10.0', 'torchvision==0.18.0', 'torchaudio==2.10.0', '--index-url', "https://download.pytorch.org/whl/cu$candidateTag")
...
try {
    ...
    if ($parsed['--cpu-mode']) {
        ...
        if (Test-TorchCpuRuntime) {
            ...
            Invoke-LoggedCommand -Description 'install torch CPU wheels' -Command $venvPython -Arguments @('-m', 'pip', 'install', '--force-reinstall', '--no-cache-dir', 'torch==2.10.0', 'torchvision==0.18.0', 'torchaudio==2.10.0', '--index-url', 'https://download.pytorch.org/whl/cpu')
        }
    }
    else {
