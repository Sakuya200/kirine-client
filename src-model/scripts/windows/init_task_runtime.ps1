...
function Install-CompatibleTorchCuda {
    ...
    foreach ($candidate in $CudaCandidates) {
        ...
        try {
            Invoke-LoggedCommand -Description "install torch wheels for cu$candidateTag" -Command $venvPython -Arguments @('-m', 'pip', 'install', '--force-reinstall', '--no-cache-dir', 'torch==2.10.0', 'torchvision', 'torchaudio', '--index-url', "https://download.pytorch.org/whl/cu$candidateTag")
