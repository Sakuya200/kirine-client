...
- `scripts/windows/toggle_lora_dependencies.ps1`: Windows LoRA 依赖切换脚本，按启用/禁用安装或卸载 `peft`。
...
- `scripts/unix/toggle_lora_dependencies.sh`: Unix-like LoRA 依赖切换脚本。
...
- `toggle-lora-dependencies`: 由设置页保存动作直接触发，按 `enable` 或 `disable` 安装/卸载 LoRA 相关依赖。
...
- `train`: LoRA 只支持显式启用或禁用。启用时必须先通过切换脚本安装 `peft`，禁用时固定走全量微调。
...
- `config.toml` 的 `[training]` 段还支持 `lora_mode`、`lora_rank`、`lora_alpha`、`lora_dropout`，并由设置页统一维护。
...
- LoRA 目标模块集合按当前 Qwen3-TTS 模型结构固定写在 `src/qwen3_tts/lora.py` 中，不通过设置页暴露，避免不同模型结构下误配。
