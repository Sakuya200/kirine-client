...
use crate::{
    common::{
        ...
        task_paths::{task_log_file_path, task_sample_dir, voice_design_params_json_path},
    },
    ...
    service::{
        ...
        pipeline::{
            ...
            run_llm_task_invocation_cancellable, run_pipeline_stage_shell_script_cancellable,
            script_paths::{
...
pub(crate) async fn run_common_voice_design_pipeline(
    ...
    let result = async {
        ...
        let paths =
            resolve_voice_design_paths(service, task_id, base_model, &params.model_version)?;
        ...
        if matches!(prepare_result, LoggedCommandResult::Cancelled) {
            mark_voice_design_cancelled_state(
                service,
                task_id,
        ...
        if !Path::new(&params.output_file_path).exists() {
            bail!(
                "Voice design output file not found after inference: {}",
                params.output_file_path
            );
        }
...
pub(crate) async fn mark_voice_design_running_state(
    service: &LocalService,
    task_id: i64,
) -> Result<()> {
    service
...
pub(crate) fn validate_voice_design_environment(
    ...
    if !Path::new(&paths.model_root_path).exists() {
        bail!(
            "Voice design model path not found: {}",
...
pub(crate) async fn run_voice_design_python_command(
    ...
    let result = run_llm_task_invocation_cancellable(
        begin_llm_task_script_path,
        voice_design_python_script_path,
        src_model_root,
