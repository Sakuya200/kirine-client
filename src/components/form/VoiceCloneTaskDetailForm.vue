...
import type { VoiceCloneHistoryRecord } from '@/types/domain';

interface Props {
  record: VoiceCloneHistoryRecord;
}
  ...
  <div class="space-y-4">
    <div class="grid gap-3 md:grid-cols-2">
      <article class="rounded-2xl border border-brand-200 bg-white/80 p-4">
        <p class="text-xs text-stone-500">{{ t('history.detailForm.refAudio') }}</p>
        <p class="mt-1 break-all text-sm font-semibold text-slate-800">{{ record.detail.refAudioName }}</p>
      </article>
      <article class="rounded-2xl border border-brand-200 bg-white/80 p-4">
    ...
    <section class="rounded-2xl border border-brand-200 bg-white/80 p-4">
      <p class="text-sm font-semibold text-slate-800">{{ t('history.detailForm.refAudioPath') }}</p>
      <p class="mt-3 break-all rounded-xl bg-brand-50/45 px-3 py-3 text-sm leading-6 text-slate-700">{{ record.detail.refAudioPath }}</p>
    </section>

    <section class="rounded-2xl border border-brand-200 bg-white/80 p-4">
      <p class="text-sm font-semibold text-slate-800">{{ t('history.detailForm.refText') }}</p>
      <p class="mt-3 whitespace-pre-wrap rounded-xl bg-brand-50/45 px-3 py-3 text-sm leading-6 text-slate-700">{{ record.detail.refText }}</p>
    </section>
