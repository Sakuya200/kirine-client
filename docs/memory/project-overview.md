...
## 核心功能模块
1. **模型管理** - 安装/卸载 6 个模型（irodori_tts_v3, dots_tts, qwen3_tts, vox_cpm2, moss_tts_local, gpt_sovits_cpufast），自动检测设备类型，配置驱动的特性矩阵。运行环境支持 venv 与 conda_env（系统 PATH 有 conda 时优先用 conda）
...
## 当前状态
- 分支 `v.0.12.0`（开发中）；Tauri 应用版本 (tauri.conf.json) `0.11.2`；Cargo.toml version `0.1.0`
- 数据库 schema version `28`；最新迁移 `m20260718_000011_add_streaming_tasks`
- 已集成 6 个模型子模块（均为独立 GitHub adapter 仓库）：irodori_tts_v3, dots_tts, qwen3_tts, vox_cpm2, moss_tts_local, gpt_sovits_cpufast
